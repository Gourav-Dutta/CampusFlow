...
// Parent are add by principle by using the add parent button like other user. This API only make a request by parent(already added) to parentStudent table.
// Only a parent make this request, Parent adding a student request - A parent can add a user as relation but the status will be pending.
 //  Only that user whom the request is through approved it
...
export async function GET( // Fetch all the parents of a particular student: Based on student id
  req: Request,
) {
  try {
        const session = await auth.api.getSession({
...
export async function PUT(
  ...
  // Only for user-student type-- the user_id  should be the user not the parent
  try {
        const session = await auth.api.getSession({
          headers: await headers(),
        });
    ...
    const updateStatus = await prisma.parentStudent.updateMany({
      where: { parent_id: parentId, student_id: userId},
      data: { status: status },
    });
    return NextResponse.json(
      {
