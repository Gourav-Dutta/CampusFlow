...
export async function POST(req: Request) {
  try {
    ...
    const validateSchoolRoom = await prisma.schoolRoom.findUnique({
      where: { id: schoolRoomId },
    });
    if (!validateSchoolRoom)
      return NextResponse.json(
        { error: "School room not found" },
        { status: 404 },
      );

    if (validateSchoolRoom.school_id !== schoolId)
      return NextResponse.json(
        { error: "The school room does not belong to the specified school" },
        { status: 400 },
      );
...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json(
      { error: "An error occurred while fetching the association" },
      { status: 500 },
    );
  }
}
